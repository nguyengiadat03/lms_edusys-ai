...
const { curriculumService } = require('../services/curriculumService.js');
...
router.get('/', [
  ...
  if (status) {
    statusFilter = typeof status === 'string' ? status.split(',').map(s => s.trim()) : [String(status)];
  }
  ...
  for (const s of statusFilter) {
    if (!validStatuses.includes(s)) {
      throw createError(`Invalid status value: ${s}`, 'VALIDATION_ERROR', 422);
    }
  }
  ...
  const result = await curriculumService.list(filters, BigInt(user.tenant_id));
...
router.post('/', [
  ...
  const framework = await curriculumService.create(frameworkData, BigInt(user.id), BigInt(user.tenant_id));

  // Log audit
  auditLogger.create(user.id.toString(), 'curriculum_framework', framework.id.toString(), frameworkData);
...
router.patch('/:id', [
  ...
  await curriculumService.update(id, updateData, BigInt(user.id), BigInt(user.tenant_id));

  // Log audit
  auditLogger.update(user.id.toString(), 'curriculum_framework', id, updateData);
